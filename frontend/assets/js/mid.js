    ...
    function filterData() {
      ...
      if (filters.types === "all" && filters.status === "all") {
        const filteredData = rows.filter((row) => {
          ...
          if (
            filters.area &&
            (row.c[11].v < filters.area.min || row.c[11].v > filters.area.max)
          ) {
            return false;
          }
          ...
          if (
            filters.price &&
            (row.c[13].v < filters.price.min || row.c[13].v > filters.price.max)
          ) {
            return false;
          }
          ...
          if (
            filters.rooms &&
            (row.c[6].v + row.c[7].v < filters.rooms.min ||
              row.c[6].v + row.c[7].v > filters.rooms.max)
          ) {
            return false;
          }
...
function createTable(rows) {
  ...
  rows.map((row) => {
    ...
    if (row.c[7].v === 1) {
      tableBody.innerHTML += `
              <tr class='units-rows' id='r-${row.c[4].v}'>
                <td class="unit-id">${row.c[4].v}</td>
              <td class="unit-area">${row.c[11].v}m2</td>
              <td class="unit-price">${row.c[13].v}</td>
              <td class="unit-room">${row.c[6].v}+1</td>
              <td class="unit-type">${row.c[14].v}</td>
              </tr>`;
    } else {
      tableBody.innerHTML += `
      <tr class='units-rows' id='r-${row.c[4].v}'>
      ...
      <td class="unit-area">${row.c[11].v}m2</td>
      <td class="unit-price">${row.c[13].v}</td>
      <td class="unit-room">${row.c[6].v}</td>
      <td class="unit-type">${row.c[14].v}</td>
      </tr>`;
    }
...
function createLabels(rows) {
  ...
  rows.map((row) => {
    ...
    spanElem.addEventListener("mouseover", function () {
      ...
      document.querySelector(`#r-${row.c[4].v}`).classList.add("active");
    });
    spanElem.addEventListener("mouseout", function () {
      console.log("ouy");
      this.classList.remove("anime");
      document.querySelector(`#r-${row.c[4].v}`).classList.remove("active");
    });
    spanElem.addEventListener("click", function () {
      ...
      document.querySelector(`#r-${row.c[4].v}`).classList.add("clicked");
      // console.log(tableRow);
      const label = document.querySelector(`#d2-${row.c[4].v}`);
      const labelPosition = label.getBoundingClientRect();
      const imagePosition = label.parentElement.getBoundingClientRect();
      const labelTop =
      ...
      const unitArea = document.querySelector(
        `#r-${row.c[4].v} .unit-area`
      ).innerText;
      const unitRoom = document.querySelector(
        `#r-${row.c[4].v} .unit-room`
      ).innerText;
      console.log(unitId, unitArea, unitRoom);
      popUp.style.display = "flex";
      popUp.style.top = `${labelTop}%`;
